...
class SateCollector {
  ...
  void start({
    ...
    required String tag,
    bool verbose = false,
    bool enableTimeSeries = false,        // Enable time-series
    int coverageInterval = 30,            // Interval in seconds
    String? lcovPath,                     // Path to lcov.info
  }) {
    _verbose = verbose;
    ...
    FlutterError.onError = (FlutterErrorDetails details) {
      _log('Flutter error caught: ${details.exception}', level: 'ERROR');
      _recordFault(
        severity: 'E+',
    ...
    PlatformDispatcher.instance.onError = (error, stack) {
      _log('Platform error caught: $error', level: 'ERROR');
      _recordFault(
        severity: 'FATAL',
  ...
  void _recordCoverageSnapshot(Map<String, Map<String, double>> coverage) {
    ...
    for (var entry in coverage.entries) {
      ...
      for (var metric in metrics.keys) {
        metrics[metric]!['covered'] = (metrics[metric]!['covered'] ?? 0) + (data['covered'] ?? 0);
        metrics[metric]!['total'] = (metrics[metric]!['total'] ?? 0) + (data['total'] ?? 0);
      }
    }
    ...
    _log('   📊 Recorded ${metrics.length} metrics at timestamp $timestamp');
  }

  /// Record a screen navigation
  void recordScreen(String screenName, {String? previousScreen}) {
    if (!_isRunning) {
      _log('⚠️ Attempted to record screen while collector not running', level: 'WARNING');
      return;
    }
  ...
  void _recordFault({
    ...
    if (!_isRunning) {
      _log('⚠️ Attempted to record fault while collector not running', level: 'WARNING');
      return;
    }
    ...
    _events.add(event);
    _log('⚠️ Fault recorded: [$severity] $exception - ${message.substring(0, message.length > 50 ? 50 : message.length)}...');
  }
  ...
  void recordCoverage({
    ...
    if (!_isRunning) {
      _log('⚠️ Attempted to record coverage while collector not running', level: 'WARNING');
      return;
    }
    ...
    _events.add(event);
    _log('📊 Coverage recorded: $metric = ${(rate * 100).toStringAsFixed(1)}% ($covered/$total)');
  }
